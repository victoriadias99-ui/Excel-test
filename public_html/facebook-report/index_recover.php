...
$consulta = "SELECT * FROM `log` ORDER BY `log`.`fecha` ASC";
...
foreach ($cuentas as $cuenta) {
    $params = array(
        'time_range' => array('since' => $fecha, 'until' => $fecha),
        'filtering' => array(),
        'level' => 'adset',
    );
    ...
    foreach ($result['data'] as $r) {
        $consulta = "SELECT count(id) as c  FROM `facebook_camp` WHERE `fecha` = '" . $fecha . "' AND `adset_id` = '" . $r['adset_id'] . "';";
        $stmt = $cnx->prepare($consulta);
        $stmt->execute();
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        if ($rows[0]['c'] == 0) {
            $consulta = "INSERT INTO `facebook_camp`(`account_id`, `account_name`, `adset_id`, `adset_name`,`campaign_id`, `campaign_name`, `spend`, `clicks`, `cpc`, `cpm`, `cpp`, `ctr`, `frequency`, `website_purchase_roas`, `website_ctr`, `fecha`) "
                    . "VALUES ('" . $r['account_id'] . "','" . $r['account_name'] . "','" . $r['adset_id'] . "','" . $r['adset_name'] . "','" . $r['campaign_id'] . "','" . $r['campaign_name'] . "','" . $r['spend'] . "','" . $r['clicks'] . "','" . $r['cpc'] . "','" . $r['cpm'] . "','" . $r['cpp'] . "','" . $r['ctr'] . "','" . $r['frequency'] . "','" . $r['website_purchase_roas'][0]['value'] . "','" . $r['website_ctr'][0]['value'] . "','" . $fecha . "')";
            $stmt = $cnx->prepare($consulta);
            $stmt->execute();
        } else {
            $consulta = "UPDATE `facebook_camp` SET "
                    ...
                    . "WHERE fecha = '" . $fecha . "' and `adset_id` = '" . $r['adset_id'] . "';";
            $stmt = $cnx->prepare($consulta);
            $stmt->execute();
        }
        $respuesta[] = $r;
    ...
    $resultData[] = $result;
    $consulta = "INSERT INTO `log`(`fecha`, `data`) VALUES ('$fecha','" . json_encode($resultData) . "')";
    $stmt = $cnx->prepare($consulta);
    $stmt->execute();
}
