...
if($topic=='payment')
{
	...
	$stmt1=$cnx->prepare("UPDATE ventas SET PAGO_ID_MP=?,ESTADO_MP=?,ESTADO_DETALLE_MP=?,FECHA_COMPRA_MP=?,FECHA_ACREDITACION_MP=?,PAGO_TIPO_MP=?,PAGO_DESCR_MP=?,PAGADOR_EMAIL_MP=?,PAGADOR_NOMBRE_MP=?,PAGADOR_APELLIDO_MP=?,PAGADOR_TIPO_MP=?,PAGADOR_ID_MP=?,METODO_PAGO_MP=?,FEE_MP=?,IMP_RECIBIDO_NETO_MP=? WHERE CURSO=? AND ID=?");
	...
	$stmt1->bindValue(16, $curso, PDO::PARAM_STR);
	$stmt1->bindValue(17, $extenal_reference, PDO::PARAM_STR);
	$stmt1->execute();
    ...
    //{
   	    $consulta ="SELECT CURSO,ID,FECHA,NOMBRE,APELLIDO,PREFIJO_CEL,CELULAR,EMAIL,PAGO_ID_MP,ESTADO_MP,ESTADO_DETALLE_MP  FROM ventas WHERE CURSO=? AND ID=?";   				
    	$stmt = $cnx->prepare($consulta);
    	$stmt->bindValue(1, $curso, PDO::PARAM_STR);
