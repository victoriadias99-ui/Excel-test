...
$string = file_get_contents("../plantillas/assets/data/plantillas.json");
...
function getJsonProd($data, $keys, $serch){
    foreach($data as $dat){
        if ($dat[$keys] == $serch){
            return $dat;
        }
...
$datos = getJsonProd($json_a,'codigo',$curso);
$url_base = 'https://excel-facil.com/devtesting';
...
try {   
    ...
    MercadoPago\SDK::setAccessToken('TEST-7657599955169640-042001-d169d4f2ce5874a9519f1832df14459b-746614848');
   // MercadoPago\SDK::setAccessToken($rows[0]['ACCESS_TOKEN_MP']);    
	...
	$item->picture_url = "http://excel-facil.com/assets/img/logojpg.jpg";
	...
	if(count($rows_descuento)>0)
	{
		...
		$itemdesc->picture_url = "http://excel-facil.com/assets/img/logojpg.jpg";		
	...
	$preference->notification_url = "http://excel-facil.com/src/IPN_mp.php?curso=$curso";
  ...
  "excluded_payment_types" => array(array("id" => "ticket")));
    	...
    	"success" => $url_base.$datos['urls']['exitoso'].'&monto='.$itemdesc->unit_price,
    	"failure" => $url_base.$datos['urls']['failure'],
		"pending" => $url_base.$datos['urls']['pending']
		);
