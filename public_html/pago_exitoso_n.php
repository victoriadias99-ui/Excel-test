...
<html>
                    ...
                    <div class="card card-producto mb-5">
                        <div class="card-body text-center m-4 p-4">
                            ...
                            <form  id="procederPagoForm">
                                ...
                                <p class="text-center">
                                    <a href="unirse_n.php?CURSO=<?= $idCurso ?>" class="py-3 pt-4" style="background-color: transparent;">No, gracias. No deseo aprovechar la situación</a>
                                </p>
                            </form>
                        </div>
                    </div>
                ...
                <div class="row justify-content-center align-items-center">
                    ...
                    <div class="col-md-6">
                        <div class="section-heading">
                            ...
                            <a class="btn btn-block btn-lg py-4 btn-outline-light" style="background-color:#e6007e;" href="unirse_n.php?CURSO=<?= $idCurso ?>"><b>Clickeame&nbsp;</b>👉</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script src="../a-libraries/js/jquery.validate.min.js"></script>
		...
		window.dataLayer.push({
			event: 'purchase',
			ecommerce: {
				transaction_id: <?= json_encode((string)($_GET['idVenta'] ?? ($_GET['CURSO'] ?? '') . '-' . time())) ?>,
				value: <?= floatval($monto ?? 0) ?>,
				currency: 'ARS',
				items: [{
					item_id: <?= json_encode((string)($producto['CURSO'] ?? '')) ?>,
					item_name: <?= json_encode((string)($producto['TITULO'] ?? '')) ?>,
					price: <?= floatval($monto ?? 0) ?>,
					quantity: 1
				}]
			}
		});
