...
if ( ! class_exists( 'YITH_WCMAP_Membership_Compatibility' ) ) {
	...
	class YITH_WCMAP_Membership_Compatibility extends YITH_WCMAP_Compatibility {
		...
		public function add_item_option( $options ) {
			...
			class_exists( 'YITH_WCMBS_Manager' ) && $membership_plans = YITH_WCMBS_Manager()->get_plans();
		...
		public function check_item_visibility( $visible, $item, $options ) {
			if ( isset( $options['visibility'] ) && 'plans' === $options['visibility']
			     && isset( $options['membership_plans'] ) && $this->hide_by_membership_plan( $options['membership_plans'] ) ) {
				return false;
			}
