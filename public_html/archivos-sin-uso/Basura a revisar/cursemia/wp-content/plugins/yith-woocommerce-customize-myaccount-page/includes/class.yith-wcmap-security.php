...
if ( ! class_exists( 'YITH_WCMAP_Security' ) ) {
	...
	class YITH_WCMAP_Security {
		...
		public function __construct() {
			...
			add_action( 'wp', array( $this, 'handle_recaptcha' ) );
		...
		public function enabled_recaptcha() {
			...
			if ( is_user_logged_in() || 'yes' !== get_option( 'yith-wcmap-enable-recaptcha', 'no' ) || ! is_page( wc_get_page_id( 'myaccount' ) ) ) {
				return false;
			}
		...
		public function add_recaptcha_script() {
			...
			?>
			<script src="https://www.google.com/recaptcha/api.js?hl=<?php echo $my_current_lang ?>" async defer></script>
			<?php
		}
