...
class Jet_Widget_Mobile_Menu extends Widget_Base {
	...
	protected function render() {
		...
		jet_menu_public_manager()->add_menu_advanced_styles( $menu_id );
		...
		$dropdown_opened_icon_html = $this->get_icon_html( $settings[ 'dropdown_opened_icon' ] );
		...
		$menu_options = array(
			...
			'toggleText'         => esc_attr( $settings['toggle_text'] ),
			'toggleLoader'       => filter_var( $settings['toggle_loader'], FILTER_VALIDATE_BOOLEAN ),
			'backText'           => esc_attr( $settings['back_text'] ),
			...
			'loaderColor'        => $settings['mobile_menu_loader_color'],
			'subTrigger'         => $settings['sub-menu-trigger'],
			'subOpenLayout'      => $settings['sub-open-layout'],
			'closeAfterNavigate' => filter_var( $settings['close-after-navigate'], FILTER_VALIDATE_BOOLEAN ),
		);

		$mobile_menu = sprintf(
			'<div id="%1$s" class="jet-mobile-menu jet-mobile-menu-widget" data-menu-id="%2$s" data-menu-options=\'%3$s\'><mobile-menu :menu-options="menuOptions"></mobile-menu></div>',
			'jet-mobile-menu-' . $menu_uniqid,
			$menu_id,
			esc_attr( json_encode( $menu_options, JSON_UNESCAPED_SLASHES ) )
		);
