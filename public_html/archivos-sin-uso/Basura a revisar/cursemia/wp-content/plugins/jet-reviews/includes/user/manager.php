...
class Manager {
	 ...
	 * @return [type] [description]
	 */
	public function get_current_user_data( $user_id = false ) {

		if ( ! $user_id ) {
			$user_id = get_current_user_id();
		}

		$prepared_user_data = $this->get_raw_user_data( $user_id );
		...
		return array(
			...
			'roles'            => $prepared_user_data['roles'],
	...
	public function add_new_guest( $data = array() ) {
		...
		$table_name = DB_Manager::get_instance()->tables( 'review_guests', 'name' );
