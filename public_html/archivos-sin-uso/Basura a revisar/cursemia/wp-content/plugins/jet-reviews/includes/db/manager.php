...
class Manager {
	...
	public static function maybe_modify_tables() {
		...
		/**
		 * [$post_source_column description]
		 * @var [type]
		 * Check if post_source column exist
		 */
		/*$post_source_column = $wpdb->get_results( "SELECT 'post_source' FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = 'wp_jet_reviews' AND column_name = 'post_source'" );

		if ( empty( $post_source_column ) ) {
			$wpdb->query( "ALTER TABLE wp_jet_reviews ADD post_source text NOT NULL AFTER id" );
		}*/

		$wpdb->query( "ALTER TABLE wp_jet_reviews MODIFY COLUMN author VARCHAR(255)" );
		$wpdb->query( "ALTER TABLE wp_jet_review_comments MODIFY COLUMN author VARCHAR(255)" );
	}
