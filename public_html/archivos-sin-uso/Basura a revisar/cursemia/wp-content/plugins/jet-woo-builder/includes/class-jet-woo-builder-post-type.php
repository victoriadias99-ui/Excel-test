...
if ( ! class_exists( 'Jet_Woo_Builder_Post_Type' ) ) {
	...
	class Jet_Woo_Builder_Post_Type {
		...
		public function enqueue_templates_popup( $hook ) {
			...
			wp_enqueue_script(
				'jet-woo-builder-template-popup',
				jet_woo_builder()->plugin_url( 'assets/js/template-popup.min.js' ),
				array( 'jquery' ),
			...
			wp_localize_script( 'jet-woo-builder-template-popup', 'JetWooPopupSettings', array(
				'button' => '<a href="#" class="page-title-action jet-woo-new-template">' . esc_html__( 'Create from predesigned template', 'jet-woo-builder' ) . '</a>',
			) );
		...
		public function set_body_class( $classes ) {
			...
			$custom_thankyou_page  = jet_woo_builder_shop_settings()->get( 'custom_thankyou_page' );
			...
			if (
				( 'yes' === $custom_cart_page && is_cart() ) ||
				( 'yes' === $custom_checkout_page || 'yes' === $custom_thankyou_page && is_checkout() ) ||
				( 'yes' === $custom_myaccount_page && is_account_page() )
			) {
