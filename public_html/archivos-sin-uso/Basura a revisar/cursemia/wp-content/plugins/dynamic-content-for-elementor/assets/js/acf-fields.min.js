...
var dropCap=function($target){if($target.length){$first_p=$target.html().trim();if($first_p.charAt(0)!='<'){$target.html('<span class="elementor-drop-cap"><span class="elementor-drop-cap-letter">'+$first_p.charAt(0)+'</span></span>'+$first_p.substring(1))}else{$first_p=$target.find('*:first-child').html().trim();$target.find('*:first-child').html('<span class="elementor-drop-cap"><span class="elementor-drop-cap-letter">'+$first_p.charAt(0)+'</span></span>'+$first_p.substring(1))}}};var WidgetElementsACFDCEHandler=function($scope,$){var elementSettings=get_acf_ElementSettings($scope);if(elementSettings.drop_cap){var target=$scope.find('p:first');if(!target.length){target=$scope.find('.edc-acf:first')}
dropCap(target)}
